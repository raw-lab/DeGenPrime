...
//			Values from https://www.pnas.org/doi/10.1073/pnas.95.4.1460
//	     Gibbs(temp): Returns the Gibbs of the DataSequence at temp (float).
//			Values from https://www.pnas.org/doi/10.1073/pnas.95.4.1460
...
namespace DeGenPrime
{
	...
	public:
		...
		void Print(int id, float temperature, float salt_conc, float primer_conc);
		void PrintCodes();
		...
		float MonoIonMod(float salt_conc) const;
		float NNMeltingTemperature(float salt_conc, float primer_conc) const;
		float BasicAnneal(DataSequence product, float salt_conc, float primer_conc);
